...
<div class="login-wrap" style="min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px 20px; background: #ffffff;">
    ...
    <div class="login-panel-card fade-up" style="background: var(--paper-raised); border: 1px solid var(--line); border-radius: 24px; padding: 36px 32px; max-width: 460px; width: 100%; box-shadow: 0 20px 40px -15px rgba(0, 0, 0, 0.08); text-align: center; position: relative;">
        ...
        <p style="font-size: 13.5px; color: var(--ink-soft); line-height: 1.6; margin: 0 0 24px 0; max-width: 380px; margin-left: auto; margin-right: auto;">
            Masukkan kata sandi baru untuk akun Anda
        </p>

        {{-- Error message --}}
        @if($errors->any())
            <div id="reset-error" class="alert-dismiss" style="text-align: left; margin-bottom: 18px; font-size: 13px; font-weight: 500; color: #dc2626; background: #fee2e2; padding: 10px 14px; border-radius: 8px; border: 1px solid #fca5a5;">
        ...
        <form action="{{ route('password.update') }}" method="POST" style="text-align: left;">
            ...
            <div class="field" style="margin-bottom: 24px;">
                <label style="display: block; font-size: 13px; font-weight: 600; color: var(--ink-soft); margin-bottom: 8px;">Konfirmasi Kata Sandi Baru</label>
                <input type="password" name="password_confirmation" placeholder="Ulangi kata sandi baru" style="width: 100%; padding: 12px 14px; border: 1px solid var(--line); border-radius: 10px; font-size: 14px; color: var(--ink); background: var(--paper-raised); outline: none;" required>
            </div>
